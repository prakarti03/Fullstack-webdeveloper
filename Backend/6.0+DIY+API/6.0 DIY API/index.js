...
app.get("/jokes/:id",(req,res)=>{
  ...
  const foundjokes = jokes.find((joke)=> joke.id === id);
  res.json(foundjokes);
});
//3. GET a jokes by filtering on the joke type
app.get("/filter",(req,res)=>{
  const type = req.query.type;
  const found = jokes.filter((joke)=> joke.jokeType === type);
  res.json(found);
});
...
app.put("/jokes/:id",(req,res)=>{
  ...
  found.jokeText= req.body.text;
  found.jokeType= req.body.type;
  res.json(found);
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  const found = jokes.find((joke)=> joke.id === id);
  const news = found;
  found.jokeText= req.body.text || news.jokeText;
  found.jokeType= req.body.type || news.jokeType;
  res.json(found);
});
